...
osThreadId ChassisMovementHandle;
osThreadId GimbalMovementHandle;
osThreadId UsbReceiveHandle;
...
//
    osThreadDef(ChassisMovement, chassis_movement, osPriorityAboveNormal, 0, 512);
    ChassisMovementHandle = osThreadCreate(osThread(ChassisMovement), NULL);

    osThreadDef(GimbalMovement, gimbal_movement, osPriorityHigh, 0, 512);
    GimbalMovementHandle = osThreadCreate(osThread(GimbalMovement), NULL);
	
	osThreadDef(UsbReceive, usb_receive, osPriorityRealtime, 0, 512);
    UsbReceiveHandle = osThreadCreate(osThread(UsbReceive), NULL);
